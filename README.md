...
- **Arcade** — climb a gauntlet of CPU fighters that get tougher each stage, ending in a mirror-match finale.
...
- **Frame-data combat** — attacks are data (`startup` / `active` / `recovery`, hitbox, knockback, hitstop) and a `CombatSystem` resolves world-space hitbox-vs-hurtbox each tick, replacing the old magic-frame collision check.
